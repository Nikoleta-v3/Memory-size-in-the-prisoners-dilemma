...
%DIF ADD main.tex       Sun Sep 20 20:53:38 2020
...
Equation~\DIFdelbegin \DIFdel{(\ref{eq:tournament_utility} ) }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:tournament_utility} }\DIFaddend is the average score (using Equation~\DIFdelbegin \DIFdel{(\ref{eq:optimisation_quadratic}) }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:optimisation_quadratic}}\DIFaddend ) against the set of opponents.
...
Note that Equation \DIFdelbegin \DIFdel{~(\ref{eq:tournament_utility} ) }\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:tournament_utility} }\DIFaddend can not be used here for the utility
of Gambler, and actual simulated players are used. This is done using~\cite{axelrodproject}
with 500 turns and 200 repetitions, moreover, Equation \DIFdelbegin \DIFdel{~(\ref{eq:gambler_optimisation}
)
}\DIFdelend \DIFaddbegin \DIFadd{\ref{eq:gambler_optimisation}
}\DIFaddend is solved numerically using Bayesian optimisation.
