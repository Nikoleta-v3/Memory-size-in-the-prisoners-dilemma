...
\begin{abstract}
    ...
    \textit{best response} memory-one strategies with a theory of mind about
    their opponents. Our results add to the literature that has shown that
...
extortionate strategies by considering new approaches. More specifically, by
considering best response memory-one strategies with a theory of mind about
their opponents. There are several works in the literature that have considered
strategies with a theory of mind~\cite{Han2011, Han2012, De2013, Devaine2014,
Stewart2012, Press2012}. These works defined ``theory of mind'' as intention
...
This work presents a closed form algebraic expression for the utility of a
memory-one strategy against a given set of opponents, a compact method of
identifying it's best response to that given set of opponents essentially: a
theory of mind. The aim is to evaluate whether a best response memory-one
...
in~\cite{Knight2019}. This is done in tournaments with and without self
interactions. Moreover, we introduce a framework that allows the comparison of
...
\subsection{Best response memory-one strategy in environments with noise}

Noise or error (in implementing a move, e.g. due to trembling hand or fuzzy
mind) is a crucial aspect in the context of the IPD. Given that noise is
unavoidable in real-world applications of the IPD, it’s therefore important
to take them into account.

Noise is the probability \(p_n\) that an error occured in a players actions,
thus the players \(p\) and \(q\) are now given by:
...
and the utility \(u_q(p)\) in an environment with noise is given by:
