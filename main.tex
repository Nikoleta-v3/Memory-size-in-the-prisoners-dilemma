...
which defection is a best response. Thus, laos identifying environments for which
...
conducted in~\cite{Stewart2012}. The names and a small explanation of the strategic rules is
given in the Appendix~\ref{appendix:tables}. Both values \(\frac{1}{N} \sum u\) and \( \frac{1}{N} \sum U\)
appear to match, shown in Table~\ref{table:list_stewart_plotkin}, thus we conclude that
the formulation of Theorem~\ref{theorem:tournament_utility} is correct.

\begin{table}[htbp]
    \begin{center}
    \input{tex/stewart_results.txt}
    \end{center}
    \caption{Results of memeory one strategies against the strategies in Table~\ref{table:list_stewart_plotkin}.}
...
solitary constraint is that \(p \in \R^4_{[0, 1]} \). The optimisation problem is
given by~(\ref{eq:mo_optimisation}).

\begin{equation}\label{eq:mo_optimisation}
\begin{aligned}
& \max_p: && u_q(p) = \frac{\frac{1}{2}  p  Q  p^T + c^T p + a} 
