...
\subsection{Limitations of extortionate behaviour} %TODO check that this is a good title
...
\end{figure}
% TODO Remove self interactions results. There are no longer needed as we have
% the Moran process results

\subsection{Limitations of memory size} %TODO check that this is a good title
...
\begin{figure}[!htbp]
    ...
    % TODO Nik: a legend for the lines or a description of the lines in the
    % caption?
\end{figure}

\subsection{Dynamic best response player} %TODO check that this is a good title
...
% TODO Discuss if this is the correct location
To investigate the above findings more formally a Moran process will be
considered. % TODO Add general reference text about Moran process.
In a population of \(N\) total individuals of two types, the probability that \(K\)
individuals of a given type can take over the population, the fixation
probability \(x_K\) is given by:
...
In the case considered here \(K\) is the number of best response players in the
population, so that \(N - K\) is the number of opponents.

The difference proposed here is to allow the best response player to act
...
Thus for every value of \(K\) there is a different best response player.

In the case of the best response player the transition probabilities depend on the payoff matrix \(A ^ {(K)}\) where:

\begin{itemize}
\item \(A ^ {(K)}_{11}\) is the long run utility of the best response player against itself.
\item \(A ^ {(K)}_{12}\) is the long run utility of the best response player against the opponent.
\item \(A ^ {(K)}_{11}\) is the long run utility of the opponent against the best response player.
\item \(A ^ {(K)}_{11}\) is the long run utility of the opponent against itself.
\end{itemize}

The matrix \(A ^ {(K)}\) is calculated using Eq. (\ref{eq:optimisation_quadratic})
for each value of \(K\) once the best response dynamics algorithm has calculated
...
of \(x_K\) for dynamically updating players. This is obtained over %TODO Add count of data points here
data points. The ratio shows a relatively high performance compared to a non
dynamic best response strategy the mean \(x_K\) over all values of \(K\) and all
experiments is given by % TODO Add mean
and in some cases this dynamic updating results in a 25\% increase in the
absorption probability.
As denoted before it is clear that the best response strategy does not have a
low SSE % TODO Make sure we're using consistent language here.
this is further compondounded by the ratio being above one showing that in many
...
% TODO We need to update this (as it has now been done).
...
\section{Lemma~\ref{lemma:stability_of_defection} Proof}\label{section:stability_defection}
