...
memory-one strategies when \(N=2\) has been generated and is available here. % ref
...
studied in evolutionary processes where the strategies that compose the
population can adapt and change their behaviour based on the outcomes of their
interactions at each generation. In these processes self interactions are key.
...
Solving this can done using  \textit{best response dynamics} at detailed in
Algorithm~\ref{algo:best_response_dynamics}.Best response dynamics are commonly
used in evolutionary game theory. Best response dynamics represent a class of
...
and repeatedly finds a strategy that maximises (\ref{eq:mo_evolutionary_optimisation})
using the numerical approach described in Section~\ref{section:numerical_experiments}.
A numerical example is given in Figure~\ref{fig:best_response_dynamics_results}.
...
For each of the 1643 pairs of opponents, from Section~\ref{subsection:best_response_n_2},
the best response in an evolutionary setting has been obtained, again using
Bayesian optimisation. The distribution of SSerrors for the best response using
evolutionary dynamics is given in Figure~\ref{fig:sserror_mem_one} and the
statistical summary for the distribution in Table~\ref{table:sserror_stats}.

Similarly to the results of Section~\ref{subsection:best_response_n_2}, the
evolutionary best response strategy does not behave uniformly as ZD. A larger
value of both the kurtosis and the skewness of the SSerror distribution indicate
that evolutionary best response is more adaptable than the equivalent best
response.
...
\begin{figure}[!htbp]
    \begin{minipage}{0.74\textwidth}
            ...
            \end{center}
            \caption{Distribution of SSerrors for evolutionary memory-one best responses, when \(N=2\)}
            \label{fig:sserror_mem_one}
    \end{minipage}
...
Figure~\ref{fig:behaviour_violin_plots} compares the tournament and evolutionary
best responses. Table~\ref{table:wilcoxon_tests} details that no statistically
significant differences have been found. However, from Figure~\ref{fig:behaviour_violin_plots},
it seems that evolutionary best response is less likely to forgive after mutual
defection whilst it is more likely to forgive after being tricked (higher $p_2$
median).
