...
Zero-determinant strategies are a special case of memory-one and extortionate
strategies. They chose their actions so that a linear relationship is forced
between their score and that of the opponent, ensuring that they will always
...
In a similar fashion to~\cite{Press2012} the purpose of this work is to consider a given
memory-one strategy, however whilst~\cite{Press2012} found a way for a player to
manipulate a given opponent, this work will consider a multidimensional
optimisation approach to identify the best response to a given group of
opponents. In particular, this work presents a compact method of identifying the
best response memory-one strategy against a given set of opponents and evaluate
whether it behaves extortionately, similar to ~\cite{Press2012}.

Further theoretical and empirical results of this work include:

\begin{enumerate}
    \item The behaviour of a best response memory-one strategy and 
...
\begin{theorem}\label{theorem:quadratic_form_u}
    ...
    \end{equation}
    and the constant terms by \(a = \input{tex/numerator_constant}\) and
...
Figure~\ref{fig:hypothesis}, thus we can not consider the utility against the mean
player instead on (\ref{eq:tournament_utility}).
...
intractable and numerical methods taking advantage of the structure will be used
...
derivative is negative, then the utility of the player maximises their utility
...
This section covers numerical experiments. The results of these experiments rely
on estimating best responses, but as stated in
Section~\ref{section:best_response_mem_one}, estimating best responses
analytically can quickly become an intractable problem. As a result, best responses will be
estimated heuristically using Bayesian optimisation~\cite{Mokus1978}.
...
a bayesian understanding of the objective function which it is well suited to the
...
Now that Bayesian optimisation has been introduced the following sections focus
on three experiments. In Section~\ref{subsection:best_response_n_2} the behaviour
of a large set of memory-one best responses is evaluated. In Section
\ref{subsection:best_respnse_evolutionary_setting} a similar set is evaluated
but this time best responses consider self interactions as well. Finally in
Section~\ref{subsection:longer_memory_best_response} the performances of
memory-one and longer-memory best responses against a number of opponents is
compared.
...
winner is decided on the average score a strategy received and not by wins, so
...
[Knight 2019] is used. SSE is defined as how far, a strategy is from
behaving extortionate, for example a high SSE implies a non extortionate behaviour.

A large data set of best response memory-one strategies when \(N=2\) has been
generated which is available here. The data set contains a total of 1000 trials
...
interactions. Similarly to Section~\ref{subsection:best_response_n_2} this is
done so that the behaviour of evolutionary best response memory-one strategies
can be evaluated.
...
These are included in the data set. The distribution of SSE for the best response
...
instances of an optimal Gambler and a memory-one best response. For each trial
two random opponents have been selected. The distributions of their transition
probabilities are given in Figures
...
been collected which are available here.
...
strategy, thus having a shorter memory is limiting. This seems to be at odd with
the result of~\cite{Press2012} which stated that ..., but as indicated by Sections
having adaptability is crucial to optimising ones score in the IPD. Memory allows
for that adaptability, and we believe that is the advantage exhibited here by
Gambler.
...
This manuscript considers \textit{best responses} in the IPD strategies, and
more specifically, \textit{memory-one best responses}. It has transformed the
continuous problem of best responses to a discrete one and proved that there is
