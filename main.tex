...
\begin{itemize}
    ...
    \item the first player cooperated whilst the second player defected, denoted as \(CD\)
    \item the first player defected whilst the second player cooperated, denoted as \(DC\)
...
The non concavity of \(u(p)\) indicates multiple local optimal points. This
is also intuitive. The best response against a cooperator, \(q=(1, 1, 1, 1)\), is
a defector \(p^*=(0, 0, 0, 0)\). But so are the strategies \(p=(\frac{1}{2}, 0,
0, 0)\) and \(p=(\frac{1}{2}, 0, 0, \frac{1}{2})\).
The
...
have been acclaimed for their robustness against a single opponent.
...
behaviour the SSE method as described in~\cite{Knight2019} is used. More
specifically,
in~\cite{Knight2019} a point \(x^*\), in the space of memory-one strategies, is
defined as the nearest extortionate strategy to a strategy \(p\). \(x^*\) is
given by,
...
where \(\tilde{p}=(-p_1, -p_2, p_3, p_4)\) and
...
Thus, SSE is defined as how far a strategy is from behaving extortionate and a high
...
strategies. This confirms similar results such as~\cite{Hilbe2013b,
Hilbe2013, Hilbe2015, Knight2019}.
...
game theory~\cite{Matsui1992}. They represent a class of strategy updating rules, where players in
...
This manuscript presented several experimental results. All data for the results is archived in~\cite{glynatsi2019}.
These results were mainly to
...
theoretical results for the evolutionary best responses dynamics approach
discussed in Section~\ref{subsection:best_respnse_evolutionary_setting}.
