...
introduced in~\cite{Press2012}, the zero-determinant. In~\cite{Stewart2012} it
...
strategies. They chose their actions so that a linear relationship is forced
between the players' score ensuring that they will always
receive at least as much as their opponents. Zero-determinant strategies are
...
evolutionary dynamics has been questioned~\cite{adami2013, Knight2018,
Harper2015}.
...
opponents and evaluate whether it behaves extortionately, similar to
...
these states; \(p=(p_1, p_2, p_3, p_4) \in \R_{[0,1]} ^ 4\).
...
(Figure~\ref{fig:markov_chain}) whose corresponding transition matrix \(M\)
is given by (\ref{eq:transition_matrix}).
...
The long run steady state probability vector \(v\) is the solution to \(v M =
v\). The stationary vector \(v\) combined with the payoff matrices of
(\ref{equ:pd_definition}) give the expected payoffs for each player. More
specifically, the utility for a memory-one strategy \(p\) against an opponent
\(q\), denoted as \(u_q(p)\), is defined by,

\begin{equation}\label{eq:press_dyson_utility}
    u_q(p) = v \cdot (R, S, T, P).
\end{equation}

To the authors knowledge there has been no previous work which explored the form
of \(u_q(p)\) any further. This manuscript proves that \(u_q(p)\) is given by a
ratio of two quadratic forms~\cite{kepner2011}, as presented in
...
The proof of Theorem~\ref{theorem:quadratic_form_u} is given in Appendix~\ref{appendix:proof_theorem_one}.
...
\begin{figure}[!htbp]
    ...
    strategies in~\cite{Stewart2012}. A positive difference indicates that
    condition (\ref{eq:condition}) does not hold.}
...
This section focused on best responses and more specifically \textit{memory-one
best response} strategies. A \textit{best response} is the strategy which
corresponds to the most favorable outcome~\cite{Tadelis2013}, thus a memory-one
best response corresponds to a strategy \(p^*\) for which
...
strategy. The problem considered is bounded because \(p \in \R^4_{[0, 1]}\).
...
The proof is given in the Appendix~\ref{appendix:proof_lemma_four}.
...
subset \(S_q\), can done analytically. The points for any or all of \(p_i \in
\{0, 1\}\) for \(i \in \{1, 2, 3, 4\}\) are trivial, and, finding finding the
...
theory~\cite{Jonsson2005}. However, for large systems these quickly become
intractable. As a result, numerical methods taking advantage of the structure will be used
which are described in Section~\ref{section:numerical_experiments}. The rest of
...
derivative is negative, then a player maximises their utility
by playing as a Defector, and their utility would only decrease if they were
to change their behaviour. Thus, defection is the best response.
...
In an environment where defection is the best response the average payoff of a
defector is always higher than any other strategy can achieve. If we consider a
setting where in each the prevalence of each type of strategy was determined by
that strategy's success in the previous round, then in a population such that
(\ref{eq:defection_condition_one}) and (\ref{eq:defection_condition_two}) hold,
defection would prevail; thus cooperation would never occur.
...
Lemma~\ref{lemma:stability_of_defection} is last theoretical result presented
in this manuscript. The following section focuses on numerical experiments.

\section{Numerical experiments} \label{section:numerical_experiments}

The results of this section rely on estimating best responses, but as stated in
...
the objective function which it is well suited to the multiple local optimas in
the described search area of this work. Differential evolution~\cite{Storn1997}
was also considered, however it was not selected due to Bayesian being
computationally more efficient.

As an example let's consider the optimisation problem of
(\ref{eq:mo_tournament_optimisation}). Figure~\ref{bayesian_example}
illustrates the change of the utility function over iterations of the algorithm.
The default number of iterations that has been used in this work is 60. After 60
calls the convergence of the utility is checked. If the optimised utility has
changed in the last 10\% iterations then a further 20 iterations are considered.
...
generate a data set containing memory-one best responses. Furthermore, using a
method introduced in~\cite{Knight2019} the extortionate behaviour of these best
responses will be evaluated. In Section
\ref{subsection:best_respnse_evolutionary_setting} a similar set has been
generated and evaluated but this time best responses consider self interactions
as well. Finally Section~\ref{subsection:longer_memory_best_response} compares
the performances of memory-one and longer-memory best responses against a number
of opponents.

\subsection{Best response memory-one strategies for \(N=2\)}\label{subsection:best_response_n_2}

As briefly discussed in Section~\ref{section:introduction} zero-determinant
strategies have been praised for their robustness against a single opponent.
Zero-determinants extortionate behaviour reassures that the strategies will
never lose a game. Though extortion works in pairwise interactions this paper
argues that in multi opponent interactions, where the payoffs matter, strategies
trying to exploit their opponents suffer.

In comparison to zero-determinants, best response memory-one strategies that
have a theory of mind of their opponents utilise their behaviour in order to
gain the most from their interactions. he question that arises then is whether
best response strategies are the optimal because they behave in an extortionate
way, similarly to zero determinants. To estimate a strategy's extortionate
behaviour the SSE method as described in ~\cite{Knight2019} is used. SSE is
defined as how far, a strategy is from behaving extortionate, for example a high
SSE implies a non extortionate behaviour.

A data set of best response memory-one strategies when \(N=2\) has been
...
trial a set of 2 opponents is randomly generated, the memory-one best response
against them is estimated. Though the probabilities \(q_i\) of the opponents are
randomly generated, Figures~\ref{fig:first_opponents_probabilities} and
...
The SSE method has been applied to the data set and it's distribution is shown
in Figure~\ref{fig:sserror_mem_one} alongside a statistics summary in
...
extortionate the opponents. So though the best response strategy will
extortionate some opponents, it utilities it's performance by behaving in a more
adaptable way than zero-determinant strategies. This analysis is extended to an
...
Moran processes and in evolutionary processes in general where the strategies
that compose the population can adapt and change their behaviour based on the
outcomes of their interactions at each generation. In these processes self
interactions are key. In this section the formulation of best responses is
extended to evolutionary settings, and more specifically to include self
interactions.

Self interactions can be incorporated in the formulation
that has been used so far. The utility of a memory-one strategy in an
evolutionary setting is given by,
...
Finding the best response in evolutionary settings is done using \textit{best
...
setting for each 1000 pairs of opponents described in Section~\ref{subsection:best_response_n_2}.
These are included in the data set~\cite{glynatsi2019}. The distribution of SSE for the best response
...
evolutionary best responses. Though, Table~\ref{table:wilcoxon_tests} details
that no statistically significant differences have been found, from
Figure~\ref{fig:behaviour_violin_plots}, it seems that evolutionary best
response has a higher $p_2$ median. Thus, evolutionary best responses are not
just more adaptable than best responses but they are also more likely to to
forgive after mutual defection whilst it is more likely to forgive after being
tricked.
...
discussed in Section~\ref{section:introduction}, however, compared to these
previous works this paper does not evaluated the performance of zero-determinant
strategies but the performance of best responses. More specifically, the performance
of the best response memory one strategy is compared that of a longer memory strategy.
...
introduced. In this manuscript Gambler $n_1 = 2, m_1 = 1$ and $m_2 = 1$ is used
as the longer memory strategy.
...
D}\}^{16 \cup 1} \rightarrow (0, 1)_{\R}\). This can be hard coded as an element
...
at~\cite{glynatsi2019}. For each trial two random opponents have been selected.
The distributions of their transition probabilities are given in Figures
\ref{fig:first_opponents_probabilities_with_gambler} and
\ref{fig:first_opponents_probabilities_with_gambler}. A total of 120 trials have
been collected.
...
The utilities of both strategies has been plotted against each other in
Figure~\ref{fig:utilities_gambler_mem_one}. Though Gambler has an infinite
...
Gambler will always performs the same or better than a best response memory one
strategy. This seems to be at odd with the result of~\cite{Press2012} that
against a memory-one opponent having a longer memory will give a strategy any
advantage. However, against two memory-one opponents Gambler that has a longer
memory performs better than the optimal memory-one strategy, thus having a
...
\begin{figure}[!htbp]
    ...
    120 different pair of opponents.}\label{fig:utilities_gambler_mem_one}
\end{figure}

\section{Conclusion}

This manuscript considers \textit{best response} strategies in the IPD game, and
more specifically, \textit{memory-one best responses}. It has shown that there is
a compact way of identifying a memory-one best response to a group of opponents.
Moreover, it has proven that there exists a condition for which in an
environment of memory-one opponents defection is the dominant and stable choice.
...
The formulation of Theorem~\ref{theorem:tournament_utility} and the result that
the utility has a form allowed us to introduce an approach for identifying
...
optimisation problem where the quadratic are non concave. The results of
Lemma~\ref{lemma:memone_group_best_response} were also used to define a
condition for which defection is known to be a best response.

This manuscript also presented empirical results. These results were mainly to
...
In Section~\ref{subsection:longer_memory_best_response}, the performance of
memory-one strategies was put against the performance of a longer memory
strategy called Gambler. There were several cases where the Gambler would outperform
