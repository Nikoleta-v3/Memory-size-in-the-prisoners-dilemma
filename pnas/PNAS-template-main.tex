...
\(x^*\) is defined as  its nearest extortionate strategy. The distance between
the two strategies is explicitly calculated and referred to as sum of squared
errors of prediction (SSE); how far \(p\) is from behaving as a ZDs.
Thus, a high SSE implies a non extortionate behaviour. The SSE method has been applied to the data set.
