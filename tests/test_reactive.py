...
def test_eliminator_method():
    roots_for_x = opt_mo.eliminator_method(
...
def test_solve_system():
    ...
    roots_for_y = opt_mo.solve_system(
        system=system, variable=y, other_roots=other_roots, other_variable=x
    )
    assert roots_for_y == set([1])


def test_reactive_set_against_cooperator():
    opponent = [(1, 1, 1, 1)]
    solution_set = opt_mo.reactive_set(opponent)
    assert solution_set == set([0, 1])


def test_reactive_set_against_defector():
    opponent = [(0, 0, 0, 0)]
    solution_set = opt_mo.reactive_set(opponent)
    assert solution_set == set([0, 1])
...
def test_reactive_set_against_player():
    ...
    solution_set = opt_mo.reactive_set(opponent)
    assert solution_set == set([0, 0.4450664548206672, 0.31493358733410926, 1])
...
def test_result_from_numerical_experiments():
    ...
    assert solution_set == set([0, .13036776235395534, 0.4267968584197452, 1])
    assert solution == (0, 0.4267968584197452, 2.6546912335393573)
